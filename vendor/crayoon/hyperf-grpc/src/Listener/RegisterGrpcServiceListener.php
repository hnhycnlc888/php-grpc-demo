...
class RegisterGrpcServiceListener implements ListenerInterface {
    ...
    public function process(object $event): void {
        ...
        if (!empty($routes) && isset($routes[0]['POST'])) foreach ($routes[0]['POST'] as $handler) {
			...
			// $handler->route = /grpc.php.telpo.hi/hello
			/*模拟go-micro微服务注册nacos服务名dingTalk*/
            //$service = trim(current(explode(".", $handler->route)), "/");	
			/*模拟java springboot微服务注册nacos服务名omc.auth.server.sysDept*/
			$arr = explode("/", $handler->route);
			$index = strrpos($arr[1], '.');
			$str = substr($arr[1], 0, $index);
			$index = strrpos($str, '.');
			$service = substr($str, 0, $index);
